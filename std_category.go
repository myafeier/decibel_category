...
func (self *DefaultCategory)Add(category *Category)(id int64,err error){
	...
	_,err=self.engine.NewSession().Insert(category)
	return
}
func (self *DefaultCategory)Delete(id int64)(err error){
	_,err=self.engine.NewSession().ID(id).Update(&Category{State:StateDeleted})
	return
}
func (self *DefaultCategory)Update(id int64,category *Category)(err error){
	_,err=self.engine.NewSession().ID(id).Update(category)
	return
}
func (self *DefaultCategory)GetChild(categoryType CateType,id int64)(result []*Category,err error){
	session:=self.engine.NewSession().Where("cate_type=?",categoryType).And("state=?",StateOk).OrderBy("list_order")
	if id>0{
